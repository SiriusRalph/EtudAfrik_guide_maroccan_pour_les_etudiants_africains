  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        else { toast.success("Connexion réussie !"); window.location.href = "/dashboard"; }
      } else {
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <Input type={showPassword ? "text" : "password"} placeholder="Mot de passe" value={password} onChange={(e) => setPassword(e.target.value)} className="h-11 pl-10 pr-10 rounded-xl" required minLength={6} />
                ...
                <Button type="submit" disabled={loading} className="w-full h-11 bg-foreground text-background hover:bg-foreground/90 rounded-xl text-sm gap-2 mt-2">
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{isLogin ? "Se connecter" : "S'inscrire"} <ArrowRight className="w-4 h-4" /></>}
                </Button>
              </form>
